...
class CanfdIMU
{
 ...
 private:
  ...
  static void ThreadUart(CanfdIMU* self)
  {
    ...
    auto tp_accl =
        LibXR::Topic::CreateTopic<decltype(self->accl_)>(self->accl_topic_name_);
    auto tp_gyro =
        LibXR::Topic::CreateTopic<decltype(self->gyro_)>(self->gyro_topic_name_);
    auto tp_quat =
        LibXR::Topic::CreateTopic<decltype(self->quat_)>(self->quat_topic_name_);
    auto tp_eulr =
        LibXR::Topic::CreateTopic<decltype(self->eulr_)>(self->eulr_topic_name_);

    auto sub_accl = LibXR::Topic::ASyncSubscriber<Eigen::Matrix<float, 3, 1>>(tp_accl);
    auto sub_gyro = LibXR::Topic::ASyncSubscriber<Eigen::Matrix<float, 3, 1>>(tp_gyro);
    auto sub_quat = LibXR::Topic::ASyncSubscriber<LibXR::Quaternion<float>>(tp_quat);
    auto sub_eulr = LibXR::Topic::ASyncSubscriber<LibXR::EulerAngle<float>>(tp_eulr);
